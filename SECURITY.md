...
| Layer | What it does |
|---|---|
| **OAuth 2.1** (`@cloudflare/workers-oauth-provider`) | claude.ai connects through a standard OAuth flow. The "login" is the connection password (`MCP_SECRET`) that only you hold. |
...
Someone could write text in a Canvas page that tries to give Claude instructions. The
fencing above makes that harder, not impossible. The important point is what the worst
case is: because there is no write path, the worst outcome is **misleading you**, not
changing anything in Canvas.

Be alert if Claude suddenly proposes fetching a URL or sending something you didn't
ask for — check where that instruction came from.

## What your school can see

This is **not anonymous**, and isn't meant to be. Your token is personal and tied to your
Canvas user. Your school's Canvas logs show:
...
They do **not** see what you ask Claude or what Claude answers.

Your school can revoke the token at any time, and everything stops working immediately.
...
key. But schools can have separate AI policies, especially around examinations, and
...
Everything Claude reads for you becomes part of the conversation and therefore goes to
Anthropic's servers — that's the mechanism, not a bug. Whether it's used for training
depends on the privacy settings of your claude.ai account.
